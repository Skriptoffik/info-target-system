...
class TargetInfoBoard(ui.BoardWithTitleBar):
	...
	def __BuildWindow(self):
		...
		self.loadingText.SetText("Ã«—Ì Ã·» «·»Ì«‰« ...")
	...
	def __BuildStatsElements(self):
		...
		self.noModelText.SetText("Ã«—Ì «·»ÕÀ...")
	...
	def Open(self, vid, name):
		...
		try:
			...
			if not image_loaded:
				self.monsterImage.Hide()
				self.noModelText.SetText("’Ê—… €Ì— „ÊÃÊœ…: %d" % vnum)
				self.noModelText.Show()
		except:
			self.monsterImage.Hide()
			self.noModelText.SetText("Œÿ√ ›Ì «·ﬁ—«¡…")
			self.noModelText.Show()
	...
	def SetStats(self, hp, dmg_min, dmg_max, defense, exp, gold_min, gold_max, regen_pct, regen_cycle, res_sword, res_twohand, res_bell, res_dagger, res_fan, res_bow):
		try:
			self.statsTextDict["HP"].SetText("|cFFa6a6a6«·ÕÌ«…:|r |cFFff4d4d%s|r" % str(hp))
			self.statsTextDict["DAMAGE"].SetText("|cFFa6a6a6«·÷——:|r |cFFff9933%s - %s|r" % (str(dmg_min), str(dmg_max)))
			self.statsTextDict["DEFENSE"].SetText("|cFFa6a6a6«·œ›«⁄:|r |cFFe6e6e6%s|r" % str(defense))
			self.statsTextDict["EXP"].SetText("|cFFa6a6a6«·Œ»—…:|r |cFF33ccff%s|r" % str(exp))
			self.statsTextDict["GOLD"].SetText("|cFFa6a6a6«·Ì«‰€:|r |cFFFFD700%s ~ %s|r" % (str(gold_min), str(gold_max)))
			
			race_str = localeInfo.TARGET_INFO_MAINRACE % localeInfo.TARGET_INFO_NO_RACE
			self.statsTextDict["RACE_MAIN"].SetText("|cFFa6a6a6«·⁄—ﬁ:|r |cFFe6e6e6%s|r" % race_str)
			
			self.statsTextDict["REGEN"].SetText("|cFFa6a6a6«· ÃœÌœ:|r |cFF66ff66%s%%|r |cFFa6a6a6ﬂ· %sÀ|r" % (regen_pct, regen_cycle))
			self.statsTextDict["RESISTS_L1"].SetText("|cFFa6a6a6”Ì›:|r |cFFffcc99%s%%|r | |cFFa6a6a6ﬁ»÷ Ì‰:|r |cFFffcc99%s%%|r" % (res_sword, res_twohand))
			self.statsTextDict["RESISTS_L2"].SetText("|cFFa6a6a6Œ‰Ã—:|r |cFFffcc99%s%%|r | |cFFa6a6a6”Â„:|r |cFFffcc99%s%%|r" % (res_dagger, res_bow))
		except Exception, e:
			pass
