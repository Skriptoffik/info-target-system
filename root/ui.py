...
class CheckBox(Window):
	...
	def CreateElements(self):
		...
		self.SetSize(self.backgroundImage.GetWidth() + 20 + self.textInfo.GetTextSize()[0] + 5, self.backgroundImage.GetHeight() + self.textInfo.GetTextSize()[1])
	...
	def SetEvent(self, func, *args):
		result = args[0] in self.eventFunc
		if result:
		...
		if self.checkImage:
			if self.checkImage.IsShow():
				...
				if self.eventFunc["ON_UNCKECK"]:
					self.eventFunc["ON_UNCKECK"](*self.eventArgs["ON_UNCKECK"])
			else:
				self.checkImage.Show()

				if self.eventFunc["ON_CHECK"]:
					self.eventFunc["ON_CHECK"](*self.eventArgs["ON_CHECK"])
